...
Bilibili 数据抓取与处理工具。给定一个用户 uid，把 27 个 B 站接口的原始响应落到本地，再把它们规整成结构化记录（视频元数据、动态、专栏正文、图文正文、UP 主画像）外加视频音频转录。

## 项目定位

- **抓取（fetching）**：异步循环 27 个端点，全局 + 端点级双层限流，412 自适应降速 + 冷却恢复，所有请求结果原样落盘。
...
uv run python -m bili_unit fetch <uid>                             # 全部 27 个端点（默认增量）
...
部分子命令统一入口未覆盖，仍要走 `bili_unit.fetching` / `bili_unit.processing`：

```bash
# 抓取子模块
uv run python -m bili_unit.fetching <uid> -q                       # 等价于 bili_unit query
uv run python -m bili_unit.fetching <uid> --delete-uid -y          # 删除某 uid 全部抓取数据（不可逆）

# 处理子模块
uv run python -m bili_unit.processing query <uid>                  # 处理任务状态（pipelines + items）
uv run python -m bili_unit.processing list-uids                    # 已处理的 uid 列表
uv run python -m bili_unit.processing video-full <uid> <bvid>      # 单视频联合视图（metadata + transcription）
```
...
| item-level fan-out | `video_detail`（自 `videos` 派生）`article_detail`（自 `articles` 派生，提供专栏 markdown 正文）`opus_detail`（自 `opus` 派生，提供图文 markdown + 图片清单）`channel_videos_season` `channel_videos_series`（自 `channel_list` 派生） |
...
| `articles` | `articles` + `article_detail`（可选） | 单篇专栏（meta + 摘要 + markdown 正文 + content_json 节点树 + word_count） |
...
测试覆盖抓取 runner、限流、所有 27 个端点的 schema 适配、处理 transform、ASR pipeline（VAD 切分、段级缓存、文本拼接）。无网络调用 — 所有外部 API 都被 mock。
