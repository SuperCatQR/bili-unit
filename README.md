...
Bilibili 数据抓取与处理工具。给定一个用户 uid，把 28 个 B 站接口的原始响应落到本地，经过解析层对象化为 typed dataclass，再把它们规整成结构化记录（视频元数据、动态、专栏正文、图文正文、UP 主画像）外加视频音频转录。

## 项目定位

- **抓取（fetching）**：异步循环 28 个端点，全局 + 端点级双层限流，412 自适应降速 + 冷却恢复，所有请求结果原样落盘。
- **解析（parsing）**：把 raw dict 对象化为 5 个 typed dataclass（`UpProfile` / `VideoDetail` / `Article` / `OpusPost` / `DynamicPost`），可选下载封面、头像、动态图片到本地。
- **处理（processing）**：从解析结果派生五类结构化记录（`video_metadata` / `dynamics` / `articles` / `opus` / `user_profile`），并对视频音频做 ASR 转录（VAD 切分 + 段级断点续传 + 段间文本去重拼接）。
...
# 解析：把 raw_payload 对象化为 typed dataclass（可选下载图片）
uv run python -m bili_unit parse <uid>                             # 解析全部 5 个 model
uv run python -m bili_unit parse <uid> -i                          # 解析 + 下载图片（头像 / 封面 / 动态图）
uv run python -m bili_unit parse <uid> -m incremental              # 增量解析（跳过已有项）

# 处理：把 typed object 转换为结构化 result
...
每个 model 实现 `from_raw()` / `to_dict()` / `from_dict()` + `collect_image_jobs()` / `apply_image_results()` 图片协议。完整字段映射见 [docs/feature/parsing.md](docs/feature/parsing.md)。
...
测试覆盖抓取 runner、限流、所有 28 个端点的 schema 适配、解析 5 个 typed model + 数据层 + command/query 编排、处理 transform、ASR pipeline（VAD 切分、段级缓存、文本拼接）。无网络调用 — 所有外部 API 都被 mock。
