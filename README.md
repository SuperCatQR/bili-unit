...
- **抓取（fetching）**：异步循环 63 个读取端点，全局 + 端点级双层限流，412 自适应降速 + 冷却恢复，所有请求结果原样落盘。
...
```bash
uv run bili-unit sync <uid>                        # 抓 raw payload 并解析为 parsed objects（加 -i 下载图片）
uv run bili-unit asr <uid>                         # ASR 转录视频音频（有完整字幕时会直接短路）
...
`tui` 目前是可移植的 line-mode dashboard：可查看 uid、当前状态、推荐动作与最近事件；用 `n` 输入新 uid 并启动 sync；用 `s/f/p/a` 触发带 preflight 的 sync/fetch/parse/asr；用 `d` 走确认删除。后续会在同一 `BiliWorkbench` 边界上替换为全屏交互界面。
