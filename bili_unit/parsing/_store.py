...
class ParsingStore:
    ...
    async def save_video_subtitle(self, sub: VideoSubtitle) -> None:
        ...
        has_official = 1 if any(not p.is_ai for p in sub.pages) else 0
        has_ai = 1 if any(p.is_ai for p in sub.pages) else 0
        await self._ctx.main.execute(
            """
    ...
    async def list_image_assets(self) -> list[dict]:
        """Return every image_asset row as a list of dicts."""
