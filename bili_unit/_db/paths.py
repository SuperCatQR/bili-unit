...
#
#   data/bili/{uid}.db        ← main DB  (consumer contract)
#   data/bili/{uid}.raw.db    ← raw DB   (producer-private)
#   data/bili/{uid}/          ← workdir  (images / audio caches; DB stores rel paths)
#
...
def resolve(uid: int, root: str | Path) -> UidPaths:
    ...
    Pass ``BiliSettings.bili_db_dir`` (or ``data/bili`` by default) as ``root``.
    """
    if uid <= 0:
