...
class UidContext:
    ...
    async def open(self, *, main: bool = True, raw: bool = True) -> None:
        """Open the requested DB connections (idempotent).

        Default keeps the old behavior: open both main and raw. Callers that
        only need the consumer-facing main DB can pass ``raw=False``.
        """
        if main and self._main is None:
            self._main = Connection(
                self._paths.main_db, kind="main", uid=self.uid,
            )
            await self._main.open()
        if raw and self._raw is None:
