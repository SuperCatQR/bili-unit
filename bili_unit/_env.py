...
class BiliSettings(BaseSettings):
    ...
    bili_fetching_stale_running_threshold_seconds: int = (
        900  # issue #3: RUNNING task with updated_at older than this is treated as PARTIAL (process killed/timeout)
    )
    ...
    bili_processing_asr_max_segment_seconds: int = 120
    # If MiMo rejects an individual segment as "high risk", retry that
    # segment by cutting it into smaller pieces.  This is a normal fallback
    # for reducing single-request scope; it does not alter the ASR prompt or
    # disguise the request payload.
    bili_processing_asr_high_risk_split_enabled: bool = True
    bili_processing_asr_high_risk_split_seconds: float = 30.0
    bili_processing_asr_high_risk_min_segment_seconds: float = 10.0
    # MiMo may return an empty transcript for a very short trailing silence.
    ...
    bili_processing_asr_cache_dir: str = "output/bili/asr/cache"
