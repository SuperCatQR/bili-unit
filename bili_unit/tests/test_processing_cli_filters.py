...
async def _seed_video_detail_payloads(
    ...
    """Write SUCCESS video_detail raw payloads + placeholder video rows.

    Mirrors the runner-test helper: the FK on ``audio_transcription.bvid``
    requires a parent ``video`` row before the spy's save call lands.
    """
...
async def test_only_bvids_filters_to_explicit_set(cmd, settings):
    ...
    await _seed_video_detail_payloads(settings, uid, all_bvids)
