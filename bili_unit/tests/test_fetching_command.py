...
async def test_command_new_uid_creates_task(stores, rl_ctl, query: Query):
    ...
    cmd = Command(ds, es, rl_ctl, fetch_fn=fetch_mock)
...
async def test_command_partial_resumes(stores, rl_ctl):
    ...
    cmd = Command(ds, es, rl_ctl, fetch_fn=AsyncMock(side_effect=fake_fetch_1))
    ...
    cmd2 = Command(ds, es, rl_ctl, fetch_fn=AsyncMock(side_effect=fake_fetch_2))
