...
async def convert_single(
    ...
    *,
    ...
    2. **Token budget / segment duration cap** (preferred when
       ``duration_seconds`` and either budget or cap information are set):
       computes per-segment length via :func:`compute_segment_seconds`.
       The optional ``max_segment_seconds`` cap is applied as a stricter
       upper bound so the ASR transcript output fits the backend's
       completion-token limit. If the clip already fits, returns the single
       full mp3.
    ...
    # Step 2: prefer duration-aware segmentation when caller supplied audio
    # duration plus either token-budget info or a transcript-safety segment
