...
3. [uid-level 端点数据](#3-uid-level-端点数据已文档化-22-个) — 已文档化 22 个 / 共 33 个
...
```
{
  "endpoint": str,
  "item_id": "",
  "payload": dict | list | str | null,  // 见下方各端点定义
  "fetched_at_ms": int                  // epoch 毫秒
}
```

**item-level 端点单条**（`raw_payload.endpoint = <endpoint>`，`item_id = bvid/cvid/...`）：
...
**endpoint 状态**存储在主库 `fetch_endpoint_state`：

```
{
  "endpoint": str,
  "status": str,              // PENDING / RUNNING / SUCCESS / PARTIAL_ITEM / FAILED_*
  "retry_count": int,
  "last_error_id": int | null,
  "item_progress": dict | null,
  "progress": dict | null,
  "updated_at_ms": int
}
```
...
凭据按端点生效：`credential_required=False` 表示 fetching 会尝试匿名请求；不等于上游保证匿名可用。隐私、风控、登录态变化可能导致单端点失败。
...
parsing 消费字段：`mid`, `name`, `sex`, `face`, `sign`, `birthday`, `level`, `vip`, `jointime`
...
parsing 消费字段：`following`, `follower`, `whisper`, `black`
...
parsing 消费字段：`archive.view`（兼容 `archive` 为 int 的情况）, `article.view`（同上）, `likes`
...
parsing 消费字段：`video` / `video_count`, `article` / `article_count`, `opus` / `opus_count`（代码做版本兼容）
...
API: `User.get_articles(pn, ps, order)` → `https://api.bilibili.com/x/space/wbi/article`
...
API: `User.get_opus(type, offset)` → `https://api.bilibili.com/x/polymer/web-dynamic/v1/opus/feed/space`
...
上游标记该接口返回为未处理数据。本项目只把它作为
`channel_videos_season` / `channel_videos_series` 的 ID 派生源。
...
API: `User.get_user_fav_tag(pn, ps)` → `https://api.bilibili.com/x/space/tag/sub/list`
...
已知限制：bilibili-api-python 当前源码未向上游传递 `pn` / `ps`，实际是单页快照。

#### 3.15 upower_qa（分页 anchor）

API: `User.get_upower_qa_list(anchor)` → `https://api.bilibili.com/x/upower/qa/list`
...
API: `User.get_user_medal()` → `https://api.live.bilibili.com/xlive/web-ucenter/user/MedalWall`

raw_payload 是上游返回的 dict；字段随登录态、隐私设置和直播侧接口版本变化。

**all_followings**（非分页，需凭据）

API: `User.get_all_followings()` → `https://api.bilibili.com/x/web-interface/attentions`
...
API: `User.get_elec_user_monthly()` → `https://api.bilibili.com/x/ugcpay-rank/elec/month/up`
...
`article_detail` 只抓 legacy 专栏正文。fetching 会跳过明显的新版笔记条目
（例如 `template_id=4`, `origin_template_id=5`,
`category.id in (41, 42)`）。同一内容出现在 `opus` 列表并能提取
`opus_id` 时，正文由 `opus_detail` 抓取。
...
fetching 层通过这些父子关系，从列表端点获取 item ID 清单，再从 item-level 端点获取详情。
其中 `article_detail` 过滤已识别的新版笔记 / 图文条目；这类内容如果也在
`opus` 列表中出现，则由 `opus_detail` 抓取正文。
...
- `opus.modules` 在 B 站 API 不同版本中可能是 dict 也可能是 list-of-dicts，parsing 层做兼容。
- B 站新版笔记 / 图文会同时出现在 `articles` 和 `opus` 列表中；legacy
  `article_detail` 不再抓取这类正文，避免把 Opus 化页面误报为大量
  `readInfo` 缺失失败。
- `user_fav_tag` 当前上游调用不传递 `pn` / `ps`，实际只返回第一页数据。
...
下列 34 个端点已在 `bili_unit/fetching/_endpoint_catalog.py` 注册，可被 `fetch` 命令调用，但 raw_payload schema 未在本文录入。
