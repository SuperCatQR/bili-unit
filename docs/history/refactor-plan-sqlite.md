...
- 记录 SQLite deliverable、删除 Python query API、一 uid 一库等设计决策
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| 外部调用方还在用 `BiliQuery` | 读侧切了直接 break | 调用方需要改用 `sqlite3.connect(bili_unit.db_path(uid))`；本仓库按 major version 标记该破坏性变更 |
| 历史 `data/bili/` 数据不可读 | 用户损失抓取成果 | (a) 文档标明 v2 不兼容 v1；(b) 提供 `tools/migrate_jsonkv_to_sqlite.py` 兜底 |
