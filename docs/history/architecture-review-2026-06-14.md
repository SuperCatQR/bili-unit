...
3. 历史决策记录 —— 不可逆的架构决策（存储后端、transform 删除、pipeline_executor seam；ContentPost 共存记录已 superseded —— PR #17 删了 ContentPost 与 selectors）
...
测试基线：545 → 555（+10）；ruff 0 错。文档同步更新：`docs/feature/{fetching,parsing,processing,manifest}.md`、`docs/structure/fetching-contract.md`、`docs/structure/bili.md` §6、`docs/api.md`、`README.md`。

## 2026-06-14 follow-up #2 — content_post 删除与 parsing 输出质量

第二轮 follow-up 落地后，回头看 `content_post/` 这个统一内容视图——没有任何下游消费它，**53% 的解析层落盘是为想象中的读者付的预付款**。同时检查实际抓回的数据，发现 `OpusPost.markdown` 90% 字节是 `Opus.markdown()` 自动塞的 raw modules YAML（avatar 层 / vip 徽章），`OpusPost.cover` 在 B 站 listing 端点偶发返回 `{url, width, height}` dict 时被 `_str_or_empty` 字符串化、导致 17/19 张封面下载失败，`VideoSubtitle` 没有显式 AI 字幕标记。
...
文档同步更新：`docs/feature/parsing.md`（OpusPost / VideoSubtitle 字段表 + 图片协议表 + DynamicPost docstring）、`README.md`、`CONTEXT.md`、本文。
