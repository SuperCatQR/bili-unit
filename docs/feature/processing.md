...
- CLI command is `asr <uid>`; `process <uid>` is kept as a compatibility alias.
...
`asr --dry-run` 返回/打印 candidate 列表和估算，不触发字幕短路、不下载音频、不调用 ASR。`--max-audio-seconds` / `--max-audio-tokens` 是预执行预算阀门：如果估算超过阈值，runner 将 audio pipeline 标记为 `PARTIAL`，返回 `budget_exceeded`，并跳过 worker dispatch，避免真实 ASR 费用发生。
...
uv run python -m bili_unit asr <uid> --dry-run             # 列出 candidate 和估算，不真跑 worker
...
| BILI_PROCESSING_ASR_BACKEND | mimo | ASR 后端选择（mimo / mock / whisper）；CLI `asr -b mock` 临时覆盖 |
