...
- CLI command is `asr <uid>`; `process <uid>` is no longer exposed by the CLI.
- The implementation package is still `bili_unit.processing` during the rename.
- The DB stage key is `asr` (`stage_task` / `stage_error`). User-facing text
  should say `asr`; `processing` describes the implementation package/store.
...
audio 流水线通过 asyncio.Queue + worker pool 并发调度，支持 incremental / full 两种处理模式。工作项来自主库 `video` + `video_page`，字幕短路只读主库 `video_subtitle`；音频 CDN URL 由 `AudioDownloader` 在执行时通过 bilibili-api 解析。详见 [docs/feature/parsing.md](parsing.md)、[docs/schema.md](../schema.md)。
...
├── _estimate.py           # processing dry-run / budget 预估（items / pages / seconds / tokens）
├── _store.py              # ProcessingStore（写主 DB 的 audio_transcription + stage_task + stage_error）
├── command.py             # ProcessingCommand.asr_uid (process_uid compatibility alias)
...
│   └── _pipeline_executor.py  # 通用 pipeline executor + WorkItem / persistence seam
└── audio/
...
```text
command  → runner, _store, _db (UidContext), parsing._store, DTO
runner   → _store, audio, fetching.auth, parsing._store, _retry
_store   → _db (UidContext)
audio.*  → aiohttp / subprocess / bilibili_api（_mimo_backend / _downloader / _converter 三处）
```

ASR uses `ParsingStore` to read main DB `video` / `video_page` / `video_subtitle`, and `ProcessingStore` to write `audio_transcription` plus task/error state. It does not read raw DB.
...
|-----------|------------------|---------|------|
| transcription | video / video_page | bvid | 每个 bvid 一个工作项，包含所有分 P |

单个 bvid 的 audio 处理流程：
1. Read parsed page metadata from main DB (`video` + `video_page`).
...
- `pages[i].duration`：page 实际时长，优先来源：
  1. 该 page 在主库 `video_page.duration_s`（缺失时回退 `video.duration_s`）
...
`asr_uid(uid, mode)` 支持两档；`process_uid(...)` 仍保留为兼容 alias：
...
audio pipeline 在 worker dispatch 前会基于候选 bvid 的 page duration 做一次轻量估算：

```text
audio_seconds = sum(video_page.duration_s)
audio_tokens  = ceil(audio_seconds * BILI_PROCESSING_ASR_TOKENS_PER_SECOND)
```

`asr --dry-run` 返回/打印 candidate 列表和估算，不触发字幕短路、不下载音频、不调用 ASR。dry-run 会以 `DRY_RUN` 结束本次 `stage_run` 并写入 `stage_event`，但不更新 `stage_task[stage='asr']`，也不写 `audio_transcription`。`--max-audio-seconds` / `--max-audio-tokens` 是预执行预算阀门：如果估算超过阈值，runner 将 audio pipeline 标记为 `PARTIAL`，返回 `budget_exceeded`，并跳过 worker dispatch，避免真实 ASR 费用发生。

`init-mimo --test` 会在写入 `.env` 后额外发一次最小 WAV 探针，确认当前 profile / key / auth_style 能实际调用 MiMo `transcribe()`。

## 数据消费规则

audio pipeline 的工作项发现只依赖主库 `video` + `video_page`，字幕短路只依赖主库 `video_subtitle`。如需新增或更新工作项，应先重跑 fetching → parsing 让主库物化出最新解析结果。processing 不打开 raw DB，也不写回 fetching 或 parsing 状态。

## 两阶段编排

```
Phase 0  扫描     load_or_init_task → 从 main DB / parsing store 发现 bvid 工作项
...
processing 写主 DB（`{bili_db_dir}/{uid}.db`）的三张表（DDL：[main_v4.sql](../../bili_unit/_db/ddl/main_v4.sql)，表语义见 [docs/schema.md](../schema.md)）：

- `audio_transcription` —— per-bvid 转写结果（FK CASCADE → `video.bvid`）
- `stage_task[stage='asr']` —— 任务包络（pipeline rollup 在 `payload` JSON）
- `stage_error[stage='asr']` —— 错误 sink（`pipeline` / `item_type` / `item_id` 列定位）

raw DB 不读也不写；processing 只消费 main DB，并把 ASR 状态写回 main DB。音频 CDN URL 由 downloader 在 worker 执行时解析，不从 raw_payload 读取。
...
| `transcription_source` | `'MIMO-ASR'` | `MIMO-ASR` / `subtitle` / `mock` |
...
> dry-run 跳过字幕短路 — 不写 `audio_transcription` / `stage_task`，也不影响 candidate 列表；只写 run history / events。

### `stage_task[stage='asr']` payload
...
`ProcessingTaskStatus`：PENDING / RUNNING / SUCCESS / PARTIAL / FAILED_PERMANENT

`ProcessingPipelineStatus`：PENDING / RUNNING / SUCCESS / PARTIAL / FAILED_PERMANENT
...
```bash
uv run python -m bili_unit asr <uid>                       # incremental（已 SUCCESS 跳过、FAILED 重跑）
uv run python -m bili_unit asr <uid> -m full               # 全量重处理（覆盖所有 bvid 转录）
uv run python -m bili_unit asr <uid> -b mock               # 临时把 ASR 后端覆盖为 mock
uv run python -m bili_unit asr <uid> --limit 5             # 仅处理发现的前 5 个 bvid
uv run python -m bili_unit asr <uid> --only-bvids BV1...   # 仅处理给定 bvid（可与 --limit 叠加）
uv run python -m bili_unit asr <uid> --retry-failed-only   # 仅重跑 status='failed' 的 bvid（隐含 incremental）
uv run python -m bili_unit asr <uid> --dry-run             # 列出 candidate 和估算；run 状态 DRY_RUN，不更新 ASR task
uv run python -m bili_unit asr <uid> --max-audio-seconds 600
uv run python -m bili_unit asr <uid> --max-audio-tokens 4000
uv run python -m bili_unit init-mimo --test                    # 交互式 MiMo ASR 配置并探测模型（写 .env）

# 只读（无 CLI 子命令；直接 SQL）：
sqlite3 output/bili/{uid}.db "SELECT bvid, status, transcription_source, audio_tokens, seconds FROM audio_transcription;"
sqlite3 output/bili/{uid}.db "SELECT * FROM video_full WHERE bvid='BV1...';"   -- video + transcription LEFT JOIN view
sqlite3 output/bili/{uid}.db "SELECT * FROM manifest_summary;"                  -- 跨 stage 摘要
```
...
`assemble()` 根据 `BILI_PROCESSING_ASR_BACKEND`（或 `asr_backend_override`）创建对应的 ASR backend（mock / mimo），把 `credential_provider`（默认 `fetching.auth.get_credential`）注入 `ProcessingCommand`。`ProcessingCommand` 不持有 store；每次 `asr_uid`（或兼容 `process_uid`）自开自关 `UidContext`，再绑定 `ProcessingStore` + `ParsingStore` 跑一遍 audio pipeline。
...
async def process_uid(...) -> ProcessingCommandResult  # compatibility alias for asr_uid
...
| BILI_PROCESSING_TEMP_DIR | output/bili/processing/temp | 音频中间产物目录（ffmpeg m4s/mp3，成功后清理） |
| BILI_PROCESSING_ASR_CACHE_DIR | output/bili/processing/asr_cache | 段级 ASR cache 目录（断点续传；bvid 全成功时清理） |
...
| BILI_PROCESSING_ASR_BACKEND | mimo | ASR 后端选择（mimo / mock）；CLI `asr -b mock` 临时覆盖 |
...
- `whisper` ASR 后端仍只是工厂里的保留名，CLI 和文档暂不暴露。
