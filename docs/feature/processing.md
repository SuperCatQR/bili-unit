...
```text
audio_seconds = sum(video_detail.info.pages[].duration)
...
`assemble()` 根据 `BILI_PROCESSING_ASR_BACKEND`（或 `asr_backend_override`）创建对应的 ASR backend（mock / mimo），把 `credential_provider`（默认 `fetching.auth.get_credential`）注入 `ProcessingCommand`。`ProcessingCommand` 不持有 store；每次 `process_uid` 自开自关 `UidContext`，再绑定 `ProcessingStore` + `ParsingStore` 跑一遍 audio pipeline。
