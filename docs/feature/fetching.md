...
fetching 层负责从 B站 API 异步抓取指定用户的数据，支持 27 个端点类型、4 种抓取模式（none / page / cursor / anchor）、全局与端点级限流、增量扫描和 item-level fan-out。底层使用 `bilibili-api-python` 异步封装，HTTP 后端优先 `curl_cffi`，备选 `aiohttp`。
...
│   ├── client.py        # EndpointSpec 注册表、API 调用、item-level 抓取
│   ├── command.py       # 写入口：fetch_uid()
│   ├── data.py          # 文件目录 KV 存储（JSON 文件）
│   ├── env.py           # 配置管理（pydantic-settings，.env + 环境变量）
│   ├── error.py         # 错误存储（JSON 文件，per-uid）
│   ├── keys.py          # 存储 key 生成函数
│   ├── query.py         # 只读查询接口
...
│   │   ├── _endpoint.py     # _EndpointMixin._run_endpoint
...
27 个端点分为两类：uid-level（直接按 uid 抓取）和 item-level（从源端点提取 items 后逐个抓取）。
...
item-level 端点（`video_detail` / `article_detail` / `opus_detail`）共享独立的低 QPS（默认 0.2，即 5 秒间隔），避免逐项 fan-out 烧光全局配额触发 B站 412。
...
| RUNNING | 直接返回，不启动第二个 runner | 直接返回，不启动第二个 runner |
...
Runner 类通过 `class Runner(_EndpointMixin, _ItemFanoutMixin)` 组合 mixin。子模块通过 `from . import fetch_endpoint, _get_retry_delays` 访问可 mock 的委托包装，保持测试 patch target 兼容（`bili_unit.fetching.runner.fetch_endpoint`）。
...
- **全局 QPS**（默认 0.5）：所有端点共享
- **端点 QPS**（默认 0.2）：每个端点独立
- **video_detail QPS**（默认 0.2）：独立且更低
...
| task_already_running | info | uid | task RUNNING，跳过 |
...
```bash
uv run python -m bili_unit.fetching --login                    # QR 扫码登录
uv run python -m bili_unit.fetching --list-uids                # 列出所有已抓取的目标用户
uv run python -m bili_unit.fetching -q <uid>                   # 查询已有结果
uv run python -m bili_unit.fetching <uid>                      # 增量抓取所有 27 个端点（默认）
uv run python -m bili_unit.fetching --mode full <uid>          # 全量抓取
uv run python -m bili_unit.fetching --mode refresh <uid>       # 刷新模式
uv run python -m bili_unit.fetching <uid> -x video_detail      # 排除指定端点（推荐：跳过最耗时的 video_detail）
uv run python -m bili_unit.fetching <uid> -e user_info videos  # 仅指定端点（调试用，与 -x 互斥）
uv run python -m bili_unit.fetching --delete-uid <uid>         # 删除指定用户的所有数据（交互确认）
uv run python -m bili_unit.fetching --delete-uid <uid> -y      # 删除（跳过确认）
uv run python -m bili_unit.fetching -v <uid>                   # debug 日志
```

> 抓取范围默认是「全部已注册端点」。`-x/--exclude-endpoints` 是推荐的剪裁方式，
> `-e/--endpoints` 仅作调试时只跑指定端点用，二者互斥。
> 统一 CLI（`python -m bili_unit fetch <uid>`）的语义与此一致。
...
| BILI_FETCHING_GLOBAL_QPS | 0.5 | 全局 QPS 上限 |
| BILI_FETCHING_ENDPOINT_QPS | 0.2 | 端点级 QPS 上限 |
| BILI_FETCHING_VIDEO_DETAIL_QPS | 0.2 | video_detail 独立 QPS |
...
测试位置          bili_unit/tests/（12 个测试文件 + conftest）
总数              133 tests
...
- Runner `_process_single_item` 改为使用 `spec.callable` 替代硬编码的 `fetch_video_detail_item`
- Runner 向 item-level callable 传递 `_uid=uid` 参数，供需要 uid 的端点使用
