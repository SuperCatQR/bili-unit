...
fetching 写两个 SQLite 库（DDL：[main_v3.sql](../../bili_unit/_db/ddl/main_v3.sql)、[raw_v2.sql](../../bili_unit/_db/ddl/raw_v2.sql)）。表语义见 [docs/schema.md](../schema.md)。常用 CLI 入口是 `sync <uid>`，它会先跑 fetching 再跑 parsing；`fetch <uid>` 保留为 raw-only / 调试入口。
