...
│   ├── __main__.py            # 已废弃的 stage 子 CLI（统一入口在 bili_unit.__main__）
│   ├── _bilibili_adapter.py   # bilibili-api-python 调用层 + fetch_endpoint
│   ├── _endpoint_catalog.py   # 64 端点注册 + PROFILES + resolve_profile()
...
> 限流状态**仅保存在进程内内存**（不再持久化）。重启后 cooldown 重算 —— 这是 SQLite 重构时刻意做的简化（见 `docs/refactor-plan-sqlite.md` §11 决策点）。
...
fetching 写两个 SQLite 库（DDL：[main_v1.sql](../../bili_unit/_db/ddl/main_v1.sql)、[raw_v1.sql](../../bili_unit/_db/ddl/raw_v1.sql)）。表语义见 [docs/schema.md](../schema.md)。
...
uv run python -m bili_unit fetch <uid> -p parsing                  # 仅 parsing 层消费的 12 端点
uv run python -m bili_unit fetch <uid> -x video_detail             # 排除指定端点（与 -p / -e 互斥）
uv run python -m bili_unit fetch <uid> -e user_info videos         # 仅指定端点（调试用）
...
# 只读（无 CLI 子命令；直接 SQL）：
sqlite3 data/bili/{uid}.db "SELECT * FROM manifest_summary;"
sqlite3 data/bili/{uid}.db "SELECT endpoint, status, retry_count FROM fetch_endpoint_state;"
sqlite3 data/bili/{uid}.db "SELECT id, endpoint, error_type, message FROM stage_error WHERE stage='fetching' ORDER BY id DESC LIMIT 20;"
```
