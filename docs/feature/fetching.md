...
item-level 端点（`video_detail` / `article_detail` / `opus_detail` / `article_list_detail`）共享独立的低 QPS（默认 0.2，即 5 秒间隔），避免逐项 fan-out 烧光全局配额触发 B站 412。
...
uv run python -m bili_unit.fetching <uid>                      # 增量抓取所有 28 个端点（默认）
