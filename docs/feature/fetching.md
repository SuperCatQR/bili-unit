...
fetching 写两个 SQLite 库（DDL：[main_v4.sql](../../bili_unit/_db/ddl/main_v4.sql)、[raw_v2.sql](../../bili_unit/_db/ddl/raw_v2.sql)）。表语义见 [docs/schema.md](../schema.md)。常用 CLI 入口是 `sync <uid>`，它会先跑 fetching 再跑 parsing；`fetch <uid>` 保留为 raw-only / 调试入口。
...
uv run python -m bili_unit sync <uid> --parse-mode full            # 抓取后全量重物化 main DB
uv run python -m bili_unit sync <uid> -p parsing                   # 常用：仅抓 parsing 层消费端点后解析
uv run python -m bili_unit sync <uid> --models video_work          # 抓取后仅解析指定 model
uv run python -m bili_unit sync <uid> --exclude-models opus_post   # 抓取后跳过指定 model
