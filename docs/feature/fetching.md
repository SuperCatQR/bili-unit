...
fetching 层负责从 B站 API 异步抓取指定用户的数据，支持 25 个端点类型、4 种抓取模式（none / page / cursor / anchor）、全局与端点级限流、增量扫描和 item-level fan-out。底层使用 `bilibili-api-python` 异步封装，HTTP 后端优先 `curl_cffi`，备选 `aiohttp`。
...
25 个端点分为两类：uid-level（直接按 uid 抓取）和 item-level（从源端点提取 items 后逐个抓取）。
...
video_detail 使用独立的低 QPS（默认 0.2，即 5 秒间隔），避免触发 B站 412。
