...
`is_complete` 表示每个 page 都有可用字幕，但 processing audio runner 只有在字幕完整且非 AI 时才短路：B 站 AI 字幕（`is_ai=true` / `lan` 以 `ai-` 开头）会保留在 `video_subtitle` 中供查看，但不跳过 ASR。`ParsingStore.video_subtitle_is_complete(bvid)` 只表达完整性 —— 读 `video_subtitle.payload` 检查每个 page 是否带 `lan`。
