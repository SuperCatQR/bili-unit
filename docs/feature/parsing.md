...
```text
user_profile → video_work → video_subtitle → article_post → opus_post → dynamic_event → content_post
```
...
@dataclass
class ContentPost:
    content_key: str              # article:{cvid} / opus:{opus_id} / video:{bvid} / dynamic:{dynamic_id}
    kind: str                     # article / opus / video / dynamic_draw / forward
    title: str
    summary: str
    text: str
    markdown: str
...
3. 否则有 `bvid` 时用 `video:{bvid}`。
4. 否则用 `dynamic:{dynamic_id}`。
...
- `video_detail.info.bvid` 强关联到 `bvid`，`video_work` 落盘后通过 `video_posts_from_parsed` 进入 ContentPost candidate（kind=`video`）。
- `MAJOR_TYPE_ARTICLE.article.id` 指向 `article:{cvid}`。
- `MAJOR_TYPE_ARCHIVE.archive.bvid` 指向 `video:{bvid}`；archive 动态本身不会单独成为 ContentPost（已由 `video_posts_from_parsed` 提供 video candidate），但 `bvid` 优先级高于 `dynamic_id`，因此带 bvid 的动态会与同 bvid 的 video candidate 合并到 `video:{bvid}` 这一条。
- `MAJOR_TYPE_OPUS` 只有在 `opus_id` 或 `/opus/{id}` jump_url 可解析时才建立 `opus_id`。
- 转发动态保留 `forwarded_ref`，并递归抽取原动态 major 的引用。

`is_complete`（按 kind 分支）：

| kind | 完整条件 |
|---|---|
| `article` / `opus` | `markdown` 非空 |
| `video` | `title` 非空 且 `cross_refs.bvid` 非空 |
| `dynamic*` | `text` 或 `images` 非空 |
