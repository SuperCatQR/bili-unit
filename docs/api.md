...
| `bili_unit.BiliCommand` | class | 写侧统一入口，编排三 stage。方法：`fetch / parse / process / delete_uid / close`。 |
...
- 任何 `_` 前缀模块：`bili_unit._env`、`bili_unit._storage`、`bili_unit._retry`、`bili_unit._logging`
- stage 子包内部：
