...
_store       ProcessingStore锛涘啓涓?DB 鐨?audio_transcription + stage_task[stage='asr'] + stage_error[stage='asr']
...
processing.command 鈫?processing.runner 鈫?audio
                                       鈫?ParsingStore (read main DB video / video_page / video_subtitle)
...
fetching 閫氳繃 raw_payload 琛屽悜 parsing 鏆撮湶 raw 鏁版嵁锛涗笉鐩存帴鍏变韩 dataclass
...
ASR 杩涘害锛坰tage_task[stage='asr'].payload.pipelines[*].items锛?
璇锋眰鐘舵€?
